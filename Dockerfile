...
# The SQLite database lives on a mounted volume (Fly Volume at /data; a Docker volume
# when running the image locally). Create the mount point in the image and give it to
# the same unprivileged user the release runs as: when a volume is mounted empty, both
# Docker and Fly's init take the mount point's ownership from the image. Without this
# the release starts as `nobody` against a root-owned directory and dies with
# "** (Exqlite.Error) unable to open database file".
RUN mkdir -p /data && chown nobody:root /data && chmod 750 /data
# Deliberately no `VOLUME /data`: it would make Docker invent an anonymous volume for a
# plain `docker run`, so a run with no `-v` would look durable when it is not — which is
# the exact misconfiguration fly.toml warns about. Without it, forgetting the mount fails
# visibly.
