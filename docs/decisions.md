...
- `votes` rows are effectively append-only, which is what lets D-037 be a pure refusal rather than a repair. (**Scoped by D-051**: `Activities.reopen_group/2` — the All Vetoed takeover's "Add new options" exit — deletes the round's ballots and resets every `voted_at` when an all-vetoed pool reopens as a `:draft`. The lock binds *within* a round; the reopen starts a new one, and there is still no path by which a voter changes a cast ballot.)
...
- The card's photo region is `photo_frame/1`, so a missing or dead `image_url` degrades to a
  ...
  **annotated by D-052: Places/Yelp will never land.** Every commercial place API is rejected on
  its terms (D-052 §2), and rating/price/distance are precisely the fields that are unstorable at
  every provider that has them and absent from the free data the app actually uses
  (research §4.6) — so this meta row is permanently a redraw, not a pending integration.
...
- `Voting.outcome/1` has **five** returns. `ResultsComponents.outcome_section/1` and `GroupLive.Results`' `finished_headline/2` (~~`/1`~~ — it took the tally from D-049 §5 so a tie could not be announced as a clean win; **back to `/1` since D-051 §2**, because both screens now call the status-aware `Voting.outcome/2` and the tie arrives as its own `{:tie, rows}` outcome) / `finished_note/2` each grew a clause; a sixth caller must handle `:vetoes_only` or it will fall into a headline that says nobody voted.
...
D-048 §3 corrected the card and left the footer. `finished_headline/1` had no tie branch, so the same page read "Tied at the top" in the card at document y=263 and "Voting is closed and you have a winner." at y=1030. ~~It takes the tally now, through a public `ResultsComponents.tie_at_top?/1`~~ — **superseded by D-051 §2**: `Voting.outcome/2` carries the dead heat as `{:tie, rows}`, `finished_headline/1` has a clause for it, and `tie_at_top?/1` was deleted with its last caller. What stands from this section is the rule (the footer may not contradict the card), delivered then by re-deriving the tie and now by both reading one outcome — presentation-only either way: `Voting.tally/1` is untouched.
...
## D-051 — Endgame screens: a resolution is recorded rather than destroyed into, the outcome is status-aware, and the carnage list shows counts, never names

**Date:** 2026-08-10
**Status:** settled (both takeovers ship: All Vetoed, and the designed Tie takeover on the same foundation)

**Context.** A `:completed` group that could not settle itself rendered a muted one-liner on both results screens: "no consensus this time" over an all-vetoed pool, and — until D-048 §3 — an outright winner over a dead heat. Two designed replacement screens exist in the linked Claude Design project (`867b0685-278c-4ce4-ae2c-bce2135705af`), committed as [docs/design/all-vetoed.dc.html](design/all-vetoed.dc.html) and [docs/design/tie.dc.html](design/tie.dc.html) (dc-runtime docs — render them over HTTP, the poke/spin logic runs), with the plan in [docs/plans/endgame-screens.md](plans/endgame-screens.md). Both are **states of the existing results LiveViews, not new routes**, rendered only when the group is `:completed` and the endgame is unresolved.

**Decision.**

### 1. Resolution is three columns on `activity_groups`, and no vote row is ever changed by it

`resolution` (string, null: `"organizer_pick"` | `"app_pick"` | `"app_rescue"` — `Consensus.Activities.Group.resolutions/0`), `resolved_activity_id` (references `activities`, `on_delete: :nilify_all`), `resolved_at`. `Consensus.Voting.tally/1` *interprets* a recorded `"app_rescue"` — the rescued row comes back `vetoed?: false, rescued?: true` and the `winner?`/`leader?` flags follow `resolved_activity_id` on a `:completed` group — but **no vote row is deleted or rewritten**. The votes stay true; only the reading is recorded. This is deliberately *not* a fallback to the least-vetoed option, so D-034's "no fallback" clause stands: nothing here fires without the organizer pressing a button that says exactly what it does.

The writes live in `Consensus.Activities`, scope-first and organizer-matched like every other group write: `resolve_group/4` (refuses unless `:completed`, unresolved, and the activity is in the resolution's valid candidate set — tie picks must be tied-at-top survivors, a rescue must be a vetoed option; every check re-runs inside `Repo.transact/1` on a fresh read, so a stale tab's `resolution: nil` struct cannot overwrite a committed resolution), `rescue_group/2` (picks the random vetoed candidate server-side, delegates — so tests drive `resolve_group/4` deterministically), and `reopen_group/2` (all-vetoed only: back to `:draft`, resolution columns *and* `deadline_at` cleared — the old deadline has passed, and `maybe_complete_group/1` fires on every read, so leaving it set would re-complete round 2 at first render — round 1's votes deleted, every `voted_at` reset, **participants and tokens kept**, so the share link in the group chat keeps working and nobody re-enters a name). All three broadcast `{:group_updated, group}` on the group's standing topic, which is what flips every open results screen at once.

### 2. `Voting.outcome/2` supersedes `outcome/1` at both results LiveViews

`outcome/1` is status-blind — it reads only the tally's flags, so a completed, unresolved dead heat came back `{:winner, row}` for `tally/1`'s position tie-break, and the screens then had to re-derive the tie (`tie_at_top?/1`) to avoid announcing it. `outcome/2` takes the group: a `:completed` group with a recorded resolution is `{:winner, row}` (the flags already point at the resolved activity); a `:completed`, unresolved dead heat — two-plus survivors sharing the top nonzero approval count — is **`{:tie, tied_rows}`**; everything else falls through to `outcome/1`, so `:no_consensus` stays the unresolved all-vetoed answer. Both `GroupLive.Results` and `JoinLive.Results` call it, so the two screens cannot disagree about whether a dead heat was settled; `tie_at_top?/1` is deleted with its last caller.

~~What renders from `{:tie, rows}` today is honest and minimal: a "Tied at the top" card that names the standoff and picks nobody~~ — **history: the designed Tie takeover landed and replaced that stopgap card** (which survives only as `ResultsComponents.outcome_section/1`'s defensive clause, unreachable from either results screen; the stopgap existed so wiring `outcome/2` never re-opened D-048 §3's contradiction window). `{:tie, rows}` now renders `ConsensusWeb.Endgame.Tie` on both screens: the tug-of-war scene with a `TIED N-WAY` counter and a `DEAD HEAT` violet context slot, the `TIED FOR FIRST` list of exactly the tied rows, and — organizer only — the two exits. Tapping a row is a **provisional** selection (the primary flips from the design's beige "Pick a winner above" to a tangerine "Lock in <name>"); **Let the app break the tie** picks a tied row server-side (`Enum.random/1`) *at the press* and then runs the design's ~1.9s row-highlight shuffle as **server-driven state** (`send_update_after/3` ticks, 110ms apart — testable from `Phoenix.LiveViewTest` where a client hook would be invisible, and the mid-spin refusals are enforced by state the server owns), landing on `APP PICKED` still unlocked so the organizer can override by tapping the other row. Only **Lock in** writes: `resolve_group/4` with `"organizer_pick"` or `"app_pick"` by how the *current* selection was made. `#winner-tie-note` and the "Also tied" eyebrow belong to the **resolved**-tie winner card, whose note and paste-to-chat summary name the recorded tie-breaker — the organizer **by username** ("aheld picked X to break the tie"; the guest waiting copy already names the human, so the exit artifact is at least as specific, and "The organizer" survives only as the fallback for a caller with no name to pass) or the app ("The app picked X at random to break the tie") — never the pool order. Guests get the same scene and list with no dots, no state chip and no buttons — a waiting line naming the organizer, and the standing `Create your own →`. Two honesty deviations from the design's sample data: the caption "or run a second round with just these two" counts the actual tie ("these three" on a 3-way), and the design's `short` display names don't exist here, so the full option name is used in the status line and lock label. The caption is **inert as drawn** — it suggests a runoff this app doesn't have; building one is still the open question D-048 §3 left, and drawing it as a mono caption rather than a control is the design's own answer for now.

### 3. The All Vetoed takeover replaces the whole results panel, on both screens

`ConsensusWeb.EndgameComponents.takeover/2` is the single trigger both results LiveViews share: `:all_vetoed` only when `status == :completed`, the outcome is `:no_consensus`, **and no resolution is recorded** — a rescued group has a winner and renders the ordinary ending, and a mid-vote all-vetoed pool keeps the running tally (approvals can still arrive). `ConsensusWeb.Endgame.AllVetoed` (a LiveComponent, per-screen logic in its own module so the tie screen can be iterated independently) is everything inside it: the fire scene transcribed from the design SVG (nine flames and four embers on their own animation clocks, SMIL arm waves, the `flick`/`flick2`/`bob`/`ember`/`buzz` keyframes in `app.css`), the poke toy (four headline stages verbatim from the design source, wrapping at four; the `VETOES N/N` counter chip comically gains one per poke — the design's own `(6+p)/(6+p)`), THE CARNAGE list, and the exits. The organizer (`role={:organizer}`, with the `%Scope{}`) gets **Add new options** → `reopen_group/2` → `/groups/:id/options` with a flash naming the missing deadline, and **Let the app pick one at random** → `rescue_group/2`; a guest gets the same scene and carnage, a waiting line naming the organizer, and the standing `Create your own →` — never the write buttons, and a forged organizer event from a guest socket matches an ignore clause. The header context slot says `ALL VETOED` in the design's tangerine on both screens — the takeover replaces the violet band, so the slot is the one place the state is named (D-041's rule intact).

Refusals flash and never crash: the LiveComponent reports through the parent (`{:endgame_flash, kind, message}` + `:endgame_refresh`), so a stale tab that presses rescue after another tab already settled the group gets an honest sentence *and* flips to the ending it missed. A second rescue is `{:error, :already_resolved}` at the context, a flash at the screen.

### 4. THE CARNAGE shows veto **counts**, never names — a deliberate deviation from the design's sample data

The design's list rows attribute each veto to a person (`MAYA`, `DEV`, `PRIYA` in the right-hand mono slot). **D-035 makes that structurally impossible**: `Voting.tally/1` returns totals only, and no function in `Consensus.Voting` can say who vetoed what — there is nothing to render even if a screen wanted to. The same slot, in the same DM Mono treatment, carries the veto *count* instead (`1 VETO`, `2 VETOES`). Honest data in the drawn slot beats inventing an attribution query to satisfy sample copy; recorded in `DESIGN-SPEC.md` and `IMPORT-NOTES.md` §11 so the next importer does not "fix" it back toward the mockup. (Same class as the stage-2 poke line saying "restaurants": design-verbatim screen copy in the web layer, not a branch on `activity_type` — invariant 12 untouched.)

**Consequences:**

- `GroupLive.Results`' `:no_consensus` footer note no longer says "a new session with a different pool is the way forward" — false since the takeover carries its own exits (reopen the pool, or let the app pick). The branch is nearly unreachable now (an unresolved all-vetoed group renders the takeover instead; a resolved one is `{:winner, row}`) and survives only for the degenerate nilified-`resolved_activity_id` case, stating the fact and stopping.
- A group can return to `:draft` **under open tabs**: both results LiveViews follow the reopen instead of crashing (organizer → `03 review` with a flash; guest → `/` with "keep your link — it works again the moment round 2 is published"), and reload paths must keep handling a `:draft` re-read.
- Deleting a vetoed option after a rescue resolved onto it would nilify `resolved_activity_id`; it cannot happen through the UI (the pool is frozen for anything `:completed`, D-037), and `outcome/2` degrades to the flags-only reading rather than crashing if it ever does.
- The rescue's honest copy is part of the contract, not decoration: the winner card carries `#winner-rescue-note` and the paste-to-chat string opens "🎲 Every option … was vetoed, so the app picked X at random." A bare "X won!" would launder a coin flip into an earned win in the one artefact the group actually reads.
- `:vetoes_only` (part of the pool vetoed, nothing that survived carrying an approval — D-047 §3) is **not** a takeover and keeps its existing card and footer note: the design covers the all-vetoed case only, and a pool with un-vetoed survivors has nothing to rescue. And the tie takeover renders for an unresolved dead heat however the group completed — the lazy deadline sweep or **Close now** — because `outcome/2` reads status and resolution, not how `completed_at` got stamped.
- The poke toy is client-visible socket state; it writes nothing and guests get it too.
- Round 1's ballots are destroyed by `reopen_group/2` **by design** — their outcome was "everything vetoed", and the screen the organizer pressed the button on said exactly that. Participants' `voted_at` resets so the same tokens can vote round 2 (D-036 locks a ballot *within* a round; the reopen starts a new one).
- The fire scene SVG is a fixed-aspect (350:250) box bottom-centered at the hero's height, **not** a full-bleed `slice` fill — at the app's 440px column a full-bleed slice lets the width govern the scale (~1.14×), blowing the figure up and cropping the design's band of negative sky. Pinned in a comment in `all_vetoed.ex`; do not "simplify" it back to `inset-0`. The tie hero solves the same crop with the opposite lever: it keeps the doc's own full-bleed `xMidYMax slice` SVG and gives the *card* the viewBox's ratio (`height_class="aspect-[7/5]"` on `endgame_hero/1` in `tie.ex`, in place of the docs' fixed 250px) — at 7:5 a `slice` fill is exact, so the figures keep the doc's headroom at any width. Both are ways of saying the card, not the viewport width, decides the scene's scale. The tie scene's sweat beads additionally carry `opacity: 0` at rest (`.endgame-drip`, app.css): an unanimated bead — the right one's 0.9s delay window, reduced motion, any frozen frame — otherwise renders as an opaque disc straddling its head's ink outline, a frame the doc's running `drip` (opacity 0 at cycle start) never shows.
- One composition deviation is accepted rather than closed: both takeovers render in the app's full-bleed column with the global footer pinned to the viewport bottom (`Layouts.app`'s `<main>` `flex-1`, D-041), not in the docs' content-hugging phone card — so a viewport taller than a short pool's content shows a band of bare surface above the footer. Recorded with its reasoning in `DESIGN-SPEC.md`'s endgame sections; un-pinning the footer for these two states alone would break the one chrome rule every screen shares and make the footer jump when a resolution flips the takeover to the winner ending live.
- Q-8's veto-floor bullet in `open-questions.md` is still open — this entry handles the all-vetoed *outcome*, not whether vetoes should be preventable from reaching it.
- 1034 tests. `README.md`, `CLAUDE.md` and the `elixir` skill carry the count and move with it.

---

## D-052 — Assisted Add v1: lookup, not discovery, inline on `02` — and every commercial place API is rejected on terms, not price

**Date:** 2026-08-11
**Status:** settled

**Context.** On `02 add options` a typed name produced a bare card while a pasted URL produced a rich one, and the screen's own copy admitted the gap (*"Restaurant search coming soon. For now, type the name yourself or paste a link."*). The architecture was settled by [docs/research/activity-discovery.md](research/activity-discovery.md) (2026-08-09, reviewed same day); the product scope by [docs/design/assisted-add-ux-brief.md](design/assisted-add-ux-brief.md); the visuals by design frame t5 ("5 · Assisted Add") in the linked Claude Design project (`867b0685-278c-4ce4-ae2c-bce2135705af`), reviewed against the brief before building. The research's §5 asked for exactly this entry — it says "a **D-050** entry" because the log stood at D-049 when it was written; the log moved on, and this is that entry at the number it actually got.

### 1. v1 is lookup, not discovery, and the surface is inline on `02` — the research's Stage-1 Discover screen is superseded as a *surface*, deferred as a feature

v1 assumes the organizer already knows the options. The shape is match-after-submit: type "Vernick", press **Add**, the card lands bare instantly exactly as before, ONE `Consensus.Discovery.search/3` fires in the background, and a match renders a dismissible *"Is this it?"* slot attached under the card (up to 3 rows, name + street address as the disambiguator) with a one-tap **Use this** that puts the typed name onto the pasted-link rail — `source_url` set, `Consensus.LinkPreview` enriches exactly as a paste does. Deleting the assist leaves `02` working exactly as it did before it existed.

**The research's Stage 1 proposed a new `Discover` LiveView behind the `Restaurant ›` chevron; that surface is replaced by the inline slot, and the Discover screen is deferred to a discovery release.** This supersedes the research doc's *reference* surface only — everything else in it (the `Consensus.Discovery` context, the `Provider` behaviour, the config registry, the URL-only `Result`, the licence rejections, Stage-0 gating, the SSRF rule for provider-returned URLs) is adopted unchanged, and the research file itself stands as written; this entry is where the surface changed, per its brief's §7. Two reasons the inline surface is the *stronger* fit for a free, flaky backend: a missed suggestion is invisible where a failed search screen looks broken, and the organizer who already knows the name never wanted a browse screen in the first place. **"Simple autocomplete" was rejected as a trap** — UX-simple but infrastructure-heavy: Nominatim's policy forbids implementing autocomplete against it outright, and the public Overpass instance measures 1.5–14s with an undocumented shared slot pool. Honest typeahead becomes possible only after the local Overture extract (research Stage 3); it is a later upgrade, not a simpler starting point.

### 2. Google, Yelp, Foursquare, Brave and Mapbox are rejected on their *terms*, so nobody re-litigates them on price

Every commercial place API forbids storing the fields the durable `activities` row is made of (research §3, each clause read at its primary source 2026-08-09): **Google** exempts only the place ID from its caching ban and requires its logo wherever Places data displays without a Google Map (traps A); **Yelp** caps every field — "any portion of the Yelp Content" — at 24 hours, on a $229/month floor (trap D); **Foursquare** PAYG forbids server-side caching *by name*, in direct, unresolvable conflict with this repo's mandatory-caching working agreement (trap C); **Brave** forbids creating a database of Search Results and expires POI IDs in ~8 hours (trap B); **Mapbox** is "temporary use" only, with storage sold separately (trap E). A results page that persists is the product's payoff (PRD invariant 5), so these are architectural incompatibilities, not cache-TTL parameters or cost lines. **Anyone proposing "just use Google/Yelp" owes an answer to research §3, not a price sheet.** The chosen path — OpenStreetMap via Overpass, then a local Overture Places extract — costs $0 at any volume this product can reach and permits unlimited storage under ODbL/CDLA, with the URL-only retention boundary (below) as the licence firewall either way.

### 3. Overpass first, Overture the destination — the seam is the risky part, the source is the swappable part

- **`Consensus.Discovery`** is a sibling of `Consensus.LinkPreview`, not a generalisation of it: `search/3` never raises, `{:ok, []}` covers both "nothing matched" and "no provider registered" (callers cannot and must not tell them apart — failure is silence), results are capped at 3, and `available_types/0` / `attribution_for/1` are the only other public reads. Invariant 13 applies verbatim: `search/3` and `Geocoder.geocode/1` are network calls with multi-second budgets, called from `start_async/3` only.
- **The registry is data** — `config :consensus, Consensus.Discovery, providers: %{type => {module, opts}}`, read with `Map.get/3`. Prod/dev-live register **four types (`restaurant`, `bar`, `bowling`, `cinema`) to the one `Consensus.Discovery.Provider.Overpass` module with different `tags:` lists** — the research §4.4 proof that the type is data, not a code path. Test config registers `Consensus.DiscoveryStub` (a `$callers`-walking stub in `test/support/`, same idiom as the LinkPreview one, so a stub installed by a test survives into the `start_async` task). Dev registers **`Consensus.Dev.ScriptedProvider`** (`dev/support/`, compiled in `:dev` only via `elixirc_paths`): query-keyed canned answers — `vernick`/`trio`/`zahav`/`slow`/`error`/`timeout` — so every t5 state and every silent failure is reachable by typing alone; **`ASSIST_LIVE=1`** (`config/runtime.exs`; `.claude/launch.json`'s `consensus-dev-live`) swaps dev back to the real Overpass adapter.
- **The Overpass adapter** builds one QL program (query regex-escaped then QL-string-escaped — a quote in a venue name must never terminate the QL string), `[out:json][timeout:10]`, a ~10s receive budget (deliberately looser than LinkPreview's 5s — measured latency runs 1.5–14s), and a **first-class error taxonomy per trap L**: 429 (rate-limited) and 504 (the query, not the service) are distinct outcomes, never collapsed into one `:unavailable`. `website_url` comes from `website` | `contact:website` | `url`, **first *valid*** — and valid includes the SSRF guard, because OSM tags are crowd-edited (trap J): every candidate must pass `Consensus.LinkPreview.check_host/1`, **promoted from `defp` to `@doc false` public** (option 1 of the research's trap-J correction — one allowlist, never a second that drifts), with the promotion pinned and an IPv4-mapped-IPv6 bypass caught by a critic round during the build now covered in `test/consensus/link_preview_test.exs`. An invalid or blocked URL nulls the field, never drops the result.
- **The cache**: `Consensus.LinkPreview.Cache` is parameterised by `:table` and the app runs **two instances of the one module** — `Consensus.Discovery.Cache` is the second supervised child, owning its own ETS table for search results and geocodes. That is the research §4.5 trade taken as recommended (the alternative was ~120 duplicated lines, and the separate ok/error-TTL semantics is exactly where a re-derivation breaks). TTL is declared by the adapter's `cache_policy/0`, a compliance constant rather than a tuning knob; `ttl_ms: :none` bypasses the cache; and **`may_store_results?: false` makes an adapter un-registerable** — `Consensus.Discovery.validate_registry!/0` runs at `Discovery.Cache` child start and refuses to boot, turning "Foursquare is not allowed" from a memo into a fact the supervision tree enforces, with a second runtime check in `search/3` so even a runtime-mutated registry never writes a forbidden provider's results to the table.
- **Overture stays the destination.** The local SQLite extract (research Stage 3 — no network, no rate limit, ~0.4ms, permissive licence) is a config change under this contract, and the observed public-Overpass flakiness is the standing argument for scheduling it.

### 4. `Result.address` is a deliberate, display-only extension of research §4.3

The research's `Result` struct had no address field; the brief's suggestion row is "venue name + street address", the address being how a human tells three Vernicks apart. `:address` is added with exactly `:chips`' lifetime — display-only, dies with the socket, never persisted. The URL-only *retention* boundary (name + `website_url` become `Activity.name` + `source_url`; everything else evaporates) is unchanged, and it is the licence firewall: the schema has nowhere to put a rating, a price band or a lat/lon.

### 5. The search area lives on the group, is stored only on geocode success, and the prompt asks once

- Two nullable columns on `activity_groups` (migration `20260811172739`): `search_area` (the organizer's typed words, changeset-capped at 100 graphemes with no `maxlength` — invariant 11) and `search_bbox` (one `"min_lat,min_lon,max_lat,max_lon"` string — Nominatim returns a bbox as one tuple, there is exactly one reader, and a single nullable string gets the "did the geocode ever run" sentinel for free). On the **group, not the person**: the pool already names local restaurants to everyone with the link, so a neighbourhood string is a property of the dinner, not the organizer (research §4.1). No browser geolocation, no IP geolocation — both rejected there and still rejected.
- `Activities.set_search_area/3` is scope-first with the head-match ownership idiom, and **draft-gated** — `{:error, :pool_locked}` outside `:draft`, like every other pool write (D-037's clock).
- **Storage happens only on geocode success.** Save geocodes async (`Consensus.Discovery.Geocoder` over Nominatim — submit-only per policy, identifying User-Agent, successes cached ~a year because a neighbourhood's bbox is immutable, failures 5 minutes) and writes both columns together; a failed geocode stores nothing.
- **Prompt semantics (02f), exactly three states:** answered once → the group has an area and the prompt can never appear again on any mount; dismissed unanswered → suppressed for that LiveView session only, its pending-lookup queue discarded with it; geocode failure → silent collapse, area still unset, so a later add may legitimately ask again. Names typed *while the question is open* queue (`pending_ids` inside the prompt) and each fires its own lookup once Save lands — they do not silently lose their one lookup.

### 6. Confirming a suggestion overwrites the typed name — and enrichment must not overwrite it back

The brief's D3, answered as recommended and as the frame draws it (02e cross-fades "Vernick" to "Vernick Food & Drink"): **Use this takes the provider's canonical name** — that is most of the polish — with the existing `02b` editor as the undo. It sets `source_url` when the result carries a website; a website-less result still confirms usefully (name set, no enrichment). The enrichment reuses the pasted-link async verbatim **except the name**: keyed `{:link_preview, id, :never}`, so the fetched page's `og:title` fills photo/description/metadata but never replaces the name the organizer just confirmed. That is a fix, not a nicety — observed live before it: vernickphilly.com's `og:title` "Vernick Philadelphia" silently undoing the canonical "Vernick Food & Drink" a second after **Use this** (+4 probe tests pin it). **The paste path is unchanged**: a pasted URL still adopts `og:title`, because there the page is the only source of a name.

### 7. Attribution is data from the provider, and it renders in every slot state

`Provider.attribution/0` returns the line **pre-split** — `%{prefix, text, url}`, the UI links only `text` and renders `prefix` as plain text, nothing hardcoded in the LiveView. Overpass's is `"Places from "` + `"OpenStreetMap contributors"` → `https://opendatacommons.org/licenses/odbl/` (the frame and the brief's constraint 3 both point the link at the ODbL licence). It renders in all three slot states — one match, multi match, area prompt — and a `nil` provider or `nil` attribution renders no line. This is what lets a future provider carry a different obligation (or none) without touching a template.

### 8. The design record: the violet assist family, the Edit pill's white-at-rest ruling, "N yours", the chip caption, and the t5 divergences

- **The violet family is the assist's accent, a deviation from DESIGN-SPEC rule 5 ("fields carry a mint shadow at rest") scoped to assist-owned surfaces**: slot background `#F5F1FF`, shimmer stripes `#D6CCFB`/`#F1ECFE`, the enriching card's `--shadow-assist: 2px 2px 0` violet, and a violet-soft `#D6CCFB` resting shadow on the area field. The main add input keeps its mint shadow in every panel and in the app. Recorded in `DESIGN-SPEC.md`; carry it, don't "fix" it.
- **The `✎ Edit` pill is white at rest, yellow on hover/press** — reversing what the app had shipped. The frame evidence that settled it: both the original `02` frame (`docs/design/screens/1a-1-02-add-options-manual-mvp.html`) and every t5 panel draw the resting pill white with a `hover: background:#FFD84D` rule; the single yellow pill in each drawing carries the *pressed* treatment (translate + collapsed shadow), i.e. it is the same control illustrated mid-hover, not a yellow resting state. Solid yellow at rest was an app deviation; yellow remains the fill of the actual actions (Add / Save / Use this).
- **The tally bar reads "N yours", not "N added by you"** — both frames word it that way ("2 yours · 1 from friends"; "3 yours"); the longer copy was an app deviation. The "· N from friends" half stays unbuilt (co-creation is Post-MVP).
- **The chip caption is removed.** "Restaurants first. More types as we grow." appears in no t5 panel — the helper budget moved to the assist's line under the input (*"Type a name and we'll try to find its link. Or paste one yourself."*, the brief's F4 swap) — and the dashed `Bars`/`Movies` chips still say "more types later" on their own, now in t5's `quiet` treatment. **The chips stay dashed by decision**: the brief's F4 wins over the research's derive-the-chip-row-from-`available_types/0` — `available_types/0` exists and is what flips `@assist_enabled?`, but no chip went live.
- **Known, deliberate divergences from frame t5**, all recorded so nobody "fixes" them toward the mockup: **(a) the input scroll model** — the frame stages a fixed-height device with the pool sliding beneath a fixed input; the app pins the add form `sticky top-[48px]` under the global chrome header, the page stays the scroller, and the h1 + type chips scroll away above the dock. The brief's constraint 5 (the input stays reachable while a suggestion shows) is the operative clause and the sticky dock satisfies it at every viewport height; the `AssistReveal` hook scrolls a new slot into view but clamps so a card's name row never goes under the dock (frame-review §e-3: the drawing over-scrolls and the brief wins). **(b)** The area prompt gets a dismiss ✕ the drawing omits — the frame's own annotation ("Dismissible by ✕ like any suggestion") and the brief agree against the drawing. **(c)** The panels' Post-MVP garnish is not built — the friend-added "Kismet · ALEX" card, "1 from friends", the saved-group row on the 5b variant; sample garnish, same class as D-051's sample-data deviation. Variant 5b itself (an area field on `01 setup`) is not built either — the frame marks neither variant recommended and the brief's reasoning points at the in-slot prompt. **(d)** The position badge keeps the shipped mint → yellow-soft → violet-soft cycle rather than re-matching each panel's sample fills. **(e)** The back affordance is the global header's `‹` (D-041), where the panels draw an in-content back circle — mockup staging, not a target. And one non-divergence worth a line: a "needs-details nudge" reading of one panel (grey title + yellow pill) was ambiguous frame storytelling — two blind readings disagreed — and was deliberately skipped rather than guessed at.

### 9. Failure is silence and zero keystroke requests — tested invariants, not aspirations

The failure trio — no match, provider error (including 429/504), timeout — plus a crashed async task all land on the same rendering: the shimmer stops and **nothing** appears; no flash, no toast, no inline error, and the organizer never learns a lookup ran. The add form deliberately carries **no `phx-change` binding**, and no code path issues a provider or geocoder request per keystroke — a policy violation (Nominatim) and an abuse of a rate-limited volunteer endpoint (trap F / brief constraint 1). Both are pinned by `test/consensus_web/live/group_live/options_assist_test.exs`, the full state-table suite for the assist.

### 10. Stage 0 ran first, and both gates passed

- **0a — the invariant-12 source pin** (`test/consensus/activity_type_invariant_test.exs`): the source-grep allowlist test the research §4.4 called the single highest-leverage artefact in the document, landed *before* any discovery code, mutation-verified. `activity_type` in `lib/` stays on a short allowlist of moduledoc/field/cast/display/opaque-registry-key lines; a `case` or function-head branch on its value fails CI. Product invariant 2 is now checkable, not aspirational.
- **0b — the Fly-egress 403 measurement** (2026-08-11, from the production machine): **0% egress penalty** — 6/6 venue sites answered from prod exactly as from a residential connection (zahav 297ms, vernick 157ms, beddia 119ms, vetri 339ms, kalaya 143ms, sweetgreen 251ms); `yelp.com/biz` 403'd from **both** sides (a bot wall, not an egress penalty) and surayaphilly.com failed from both (excluded as a site problem). The research's kill condition — "if LinkPreview can't enrich from production, 'we find the link' delivers bare names and we should stop" — did not fire; this green light was the precondition for building Stage 1 at all.

**Consequences:**

- **Q-6 is answered and deleted from `open-questions.md`**: the provider is OSM (Overpass, then Overture), the commercial candidates are rejected on terms, and the caching question dissolves — the stored artefact is a URL plus what the venue's own page says, and ODbL/CDLA permit unlimited storage with attribution.
- **D-044's "none of it can until Places/Yelp lands (Post-MVP)" is annotated in place**: the frame's `$$$ · 4.5 ★ · 0.8 mi` meta is not waiting on a provider — rating and price band are precisely the fields that are unstorable at every commercial provider and absent from the free data (research §4.6), so that row is permanently a redraw, not a pending integration.
- The parked Discover frame (`1a-3-…`) stays parked; whenever a discovery release picks it up, its rating/price/distance elements must be redrawn per research §4.6 — that guidance transfers to any future surface.
- `Consensus.LinkPreview.check_host/1` is public API now (for the one reason above); the `function_exported?` pin means a tidy-up cannot re-privatise it without failing the suite.
- The supervision tree gained one child (`Consensus.Discovery.Cache`, directly after `Consensus.LinkPreview.Cache`), asserted by `test/consensus/application_test.exs` like every other position that matters.
- CLAUDE.md's invariant 12 now points at the source-pin test; the repo-layout tree, the narrative header and the test count moved in the same docs pass, and the stale operational claims it carried (flyctl "not logged in", "nothing has ever been deployed") were corrected against `fly auth whoami` and D-040.
- 1171 tests (from 1034), 0 failures, verified 2026-08-11.
...
- 1256 tests (from 1241), 0 failures.
...
Still undecided: the veto floor (Q-8), booking deep-links (Q-5), ~~the Places/Yelp provider and its caching terms (Q-6)~~ (**answered by D-052**: there is no Places/Yelp provider and never will be — every commercial place API is rejected on terms; the provider is OSM via Overpass, then a local Overture extract, and the URL-only retention boundary dissolves the caching question), winner delivery (Q-7), the remaining sessions/options field ambiguities (Q-12), and the whole non-blocking set (Q-13 through Q-16).
