defmodule ConsensusWeb.UserAuthTest do
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        ConsensusWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
  ...
  describe "require_admin_user/2" do
    setup %{conn: conn} do
      %{conn: conn |> fetch_flash() |> Map.put(:request_path, "/admin/users")}
    end
    ...
    test "lets an admin through", %{conn: conn} do
      ...
      conn =
        conn
        |> assign(:current_scope, user_scope_fixture(admin))
    ...
    test "sends a signed-in member home", %{conn: conn} do
      user = user_fixture()

      conn =
        conn |> assign(:current_scope, Scope.for_user(user)) |> UserAuth.require_admin_user([])
    ...
    test "sends an anonymous visitor to log in and remembers where they were going",
         %{conn: conn} do
      conn =
        conn
        |> Map.replace!(:method, "GET")
    ...
    test "refuses when no scope has been assigned at all", %{conn: conn} do
  ...
  describe "on_mount :require_admin" do
    test "continues for an admin", %{conn: conn} do
      admin = admin_fixture()
    ...
    test "halts a signed-in member and sends them home", %{conn: conn} do
      user = user_fixture()
      user_token = Accounts.generate_user_session_token(user)
      session = conn |> put_session(:user_token, user_token) |> get_session()

      {:halt, updated_socket} =
        UserAuth.on_mount(:require_admin, %{}, session, halting_socket())

      assert updated_socket.assigns.flash["error"] =~ "do not have access"
    end

    test "halts an anonymous visitor and sends them to log in", %{conn: conn} do
      session = conn |> get_session()

      {:halt, updated_socket} =
        UserAuth.on_mount(:require_admin, %{}, session, halting_socket())

      assert updated_socket.assigns.flash["error"] =~ "must log in"
    end

    test "halts a user whose admin role was revoked after the session was created",
         %{conn: conn} do
      keeper = admin_fixture()
      demoted = admin_fixture()
      user_token = Accounts.generate_user_session_token(demoted)
      session = conn |> put_session(:user_token, user_token) |> get_session()

      {:ok, {_demoted, _tokens}} =
        Accounts.set_admin(user_scope_fixture(keeper), demoted, false)
